...
SS_USER="${STORAGE_USERNAME}"  # Storage user
SS_PWD="${STORAGE_PASSWORD}"  # Storage password
SS_CONTAINER="${STORAGE_CONTAINER}"  # Storage container where we put backup files

# Backup settings
TARGET_DIR=""  # What to backup
SUPLOAD=/usr/local/bin/sbackup
BACKUP_NAME="backup"
...
"
# SQL backup settings
DB_TYPE="pgsql" # (mysql:pgsql)
DB_NAME="${DB_NAME}"  # Database name, set __ALL__ for backup all dbs or empty for disable backup
DB_USER="${DB_USERNAME}"
DB_PWD="${DB_PASSWORD}"
DB_HOST="${DB_HOST}"
DB_PORT="${DB_PORT:5432}" # Usually 3306 for MySQL and 5432 for PostgreSQL
...
STORAGE_EXPIRE="${STORAGE_EXPIRE_DAYS}"
...
if [ -n "$DB_BACKUP_FILENAME" ]; then
	...
	$BZIP -9 -f "$BACKUP_DIR/$DB_BACKUP_FILENAME"
